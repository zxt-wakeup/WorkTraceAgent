...
- 每次日报或周报生成都必须基于本期完整会话更新画像；日报每天重新扫描当天已接受的用户、助手与工具消息，优先吸收用户明确表达的工作偏好。可保留上一版仍有依据的 facet，也应在新而更强的证据出现时修订、降级或删除旧 facet。
- 更新后的画像必须反向用于当期报告的价值排序、信息密度、表达方式和建议个性化，但不在用户可见正文中单列展示。它不能证明工作完成、OKR 对齐、风险状态、Todo 或业务影响，也不能替代这些字段要求的本期 `E-xxxxxxxxxxxx` 证据。
