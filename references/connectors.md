...
默认候选根目录为 macOS 的 `~/Library/Application Support/Cursor`、`Cursor - Insiders` 和 `Cursor Nightly`；检测到任一目录时自动启用，也可在 `connectors.cursor.roots` 中显式配置产品级目录。

Connector 只读检查 `User/workspaceStorage`、`User/History` 和 `state.vscdb` 的快照，兼容已识别的 `ItemTable` 会话容器及 `cursorDiskKV` composer/bubble 明文记录。只有同时具备可识别角色、正文和周期内时间戳的明文聊天消息才进入 `ConversationTrace` 并生成 `E-` 证据锚点；同一 `composerId` 的气泡按时间归并，短消息也完整保留，thinking/reasoning 块排除。无时间戳、未知结构、工作区活动、文件历史和一般缓存片段只作 discovery 信号，不能证明工作完成。Cursor 没有稳定公开的本地 transcript Schema，因此只要启用，coverage 保守保持 `partial`；格式漂移时不得回退为猜测式完整解析。
