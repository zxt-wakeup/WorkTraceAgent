...
6. 最多生成 4 条符合 Schema 的建议，按相关性优先、时效性次之排序并写入权限受限的临时 JSON。根字段 `research_run_id`、`research_as_of`、`one_year_cutoff` 和 `aihot_scope` 必须逐字段复制生成的 selection-context；每条 `work_links` 必须从 `research-brief` 的同一条工作逐字段复制 `work_item_id`、`work_summary` 和全部 `evidence_refs`。不得靠自由文本自报关联、跨工作拼接或复用另一轮 prepare 的旧结果。每条还要在 `why_relevant` 中说明工作关联与时间选择，并包含具体优化建议、可尝试动作、边界和真实来源。
