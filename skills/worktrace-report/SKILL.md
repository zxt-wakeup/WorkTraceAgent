...
6. 交付同目录下的两个用户版本：`daily-report.md` + `daily-report.txt`，或 `weekly-report.md` + `weekly-report.txt`。日报信息充分、可扫描，并固定为`工作内容`、`工作建议`、`明日阅读`三个主板块：工作内容展示多条完整成果、带状态的进展和其他工作；每个 Todo 附简短依据；阅读项说明“资料讲什么”和“为什么推荐”。所有展示句和资料标题必须完整，不得用字符硬截断制造省略号；不显示独立`问题与对策`板块。周报目标 300–500 字；配置过往届周报时，必须按其章节标题、章节顺序和列表编号方式输出，不得退回标准版式。说明周期、覆盖状态、画像更新时间、外部研究状态和两条文件路径。
